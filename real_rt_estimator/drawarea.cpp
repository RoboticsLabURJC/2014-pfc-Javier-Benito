...
namespace real_rt_estimator {
	...
	void DrawArea::InitOGL (int w, int h) {
		...
		glClearColor(0.98f, 0.98f, 0.98f, 0.0f); /////////////////////////////////////////////////
		glClearDepth(1.0);
			...
			if (strcmp(word1,"worldline")==0){
				if (number == 9){
					if (numlines < MAX_LINES){
						...
						printf("kinect3dPoints: error, too much lines in the world file configuration.\n");
					}
				}
				else{
					printf("kinect3dPoints: error, worldfile line not valid: %s\n",buffer_file);
				}
			}
				...
				if (number==11){
					if (post_p < MAX_POST){
						...
						printf("kinect3dPoints: error, too much posts in the world file configuration.\n");
					}
				}
				else{
					printf("kinect3dPoints: error, post line not valid: %s\n",buffer_file);
				}
			}
				...
				if (number==11){
					if (jamb_p<MAX_JAMB){
						...
						printf("kinect3dPoints: error, too much jambs in the world file configuration.\n");
					}
				}
				else{
					printf("kinect3dPoints: error, post line not valid: %s\n",buffer_file);
				...
				if (number==4){
					if (circle_p<MAX_CIRCLE){
						...
						printf("kinect3dPoints: error, too much circles in the world file configuration.\n");
					}
				}
				else{
					printf("kinect3dPoints: error, circle line not valid: %s\n",buffer_file);
				}
			}
				...
				if (number==4){
					if (triangle_p<MAX_TRIANGLE){
						...
						printf("kinect3dPoints: error, too much triangles in the world file configuration.\n");
					}
				}
				else{
					printf("kinect3dPoints: error, triangle line not valid: %s\n",buffer_file);
				}
			}
				...
				if (number==4){
					if (sq_p<MAX_SQUARE){
						...
						printf("kinect3dPoints: error, too much sqs in the world file configuration.\n");
					}
				}
				else{
					printf("kinect3dPoints: error, sq line not valid: %s\n",buffer_file);
				}
			}
		}
		return 1;
...
/****************************************************************************************/
	...
	int DrawArea::read_world_file(char* worldfile) {
		FILE *myfile;
		int i;
	...
	void DrawArea::add_line(float x0,float y0, float z0, float x1, float y1, float z1) {
	...
	void DrawArea::add_camera_line(float x0,float y0, float z0, float x1, float y1, float z1) {
		if (num_camera_lines <MAX_LINES){
			...
			printf("kinect3dPoints: error, too much camera lines in the world file configuration.\n");
		}
	}

	void DrawArea::clear_camera_lines() {
		num_camera_lines=0;
	}

	void DrawArea::add_kinect_point(float x,float y, float z,int r,int g, int b) {
		jderobot::RGBPoint p;
		p.x=x;
		p.y=y;
		p.z=z;
	...
	void DrawArea::clear_points() {
		cloud.clear();
	}

	void DrawArea::clearExtraLines() {
		numextra_lines=0;
	}

	void DrawArea::setCamerasResolution(int width, int height) {
		img_width=width;
		img_height=height;
	}
